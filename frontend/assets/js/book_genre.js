...
const showBook = (arr) => {
    arr.forEach(book => {
        ...
        container_book.innerHTML +=` <div class="card_book" data-pos="${book.id}" data-rating="${book.rating["COUNT(rating)"]}" data-avg-rating="${book.rating['round(AVG(rating),1)']}">
                <div onclick="window.location.href =\`./book.php?id=${book.id}\`" class="info_book">
                    <div class="header-card">
                           <div  style="display: flex; align-items: center;">
                        <svg width="0" height="0" viewBox="0 0 32 32">
  <defs>
    <linearGradient id="half${book.id}" x1="0" x2="100%" y1="0" y2="0">
    ...
    <symbol viewBox="0 0 32 32" id="star${book.id}">
    ...
    <svg class="c-icon" width="32" height="32" viewBox="0 0 32 32">
        <use xlink:href="#star${book.id}" fill="url(#half${book.id})"></use>
    </svg>

                            <span style="margin-left: 10px">${book.rating['round(AVG(rating),1)'] ?? 0}</span>
                            </div>                        
                        <div>
                            <span>${book.rating["COUNT(rating)"] ?? 0}</span>
                            <span>отзыв.</span>
                        </div>
                    </div>
                    <div class="img_book">
